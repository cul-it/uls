...
class SmackUCIWooCommerceHelper {
	...
	public function importWooCommerceCategories($data_array,$importType,$mode,$eventKey,$duplicateHandling) {
		global $uci_admin;
		global $wpdb;
		...
		if($mode == 'Insert') {
			...
			if(!empty($display_type)) {
				$wpdb->query("insert into ".$wpdb->prefix."termmeta(term_id,meta_key,meta_value)values('".$id."','display_type','".$display_type."' ) ");
			}
			if(!empty($parent)) {
				...
				$uci_admin->setInsertedRowCount($uci_admin->getInsertedRowCount() + 1);
				$uci_admin->detailed_log[$uci_admin->processing_row_id]['Message'] = 'Inserted Product Categories ID: '.$id;
			}
			else
			{
		...
		if($mode !='Insert' && !empty($conditions)) {
			if(in_array('TERMID',$conditions)) {
				$termid=$wpdb->get_row("select term_id from wp_terms where name = '".$data_array['term_id']."'");
				if(!empty($termid)) {
					...
					if(!empty($parent)) {
						...
						$uci_admin->setInsertedRowCount($uci_admin->getInsertedRowCount() + 1);
						$uci_admin->detailed_log[$uci_admin->processing_row_id]['Message'] = 'Updated Product Categories ID: '.$id;
					}
					else {
