...
#require_once "class-uci-eshop-helper.php";
	...
	public function get_import_post_types(){
		...
		if(in_array('woocommerce/woocommerce.php', $this->get_active_plugins())){
			...
			$importas['WooCommerce Categories']='WooCommerceCategories';
	...
	public function WPCustomFields($import_type, $importAs, $mode = null) {
		...
		$add_video=array();
		$add_video[]='_format_video_embed';
		$keys=array_merge($keys,$add_video); //post_video for polimer
		foreach ($keys as $val) {
...
#$error_msg = isset($result['ERROR_MSG']) ? $result['ERROR_MSG'] : '';
		...
		case 'RF':
			...
			if(empty($acf_rf_row_mapping) || !file_exists(SM_UCI_PRO_DIR . 'helpers/class-uci-acf-data-import.php'))
				break;
			global $acfHelper;
			$acfHelper->push_acf_data($data_to_import,$duplicateHandling, $mediaConfig);//priya
			break;
		case 'GF':
			$acf_rf_row_mapping = $this->getRowMapping('GF'); 
			if(empty($acf_rf_row_mapping) || !file_exists(SM_UCI_PRO_DIR . 'helpers/class-uci-acf-data-import.php'))
				break;
			global $acfHelper;
			$acfHelper->push_acf_data($data_to_import,$duplicateHandling, $mediaConfig);//priya
	...
	if($importType != 'ticket') {
		switch ($importType) {
			...
			case 'WooCommerceRefunds':
				...
				break;
			case 'WooCommerceCategories':
				$returnArr = $helperObj->importWooCommerceCategories($data_array,$importType,$mode,$eventKey,$duplicateHandling);
				break;
			case 'WPeCommerce':
			...
			default:
				...
				if($mode == 'Schedule' && !empty($conditions)){
					if($data_array['ID']){
						$whereCondition = " ID = '{$data_array['ID']}'";
						$duplicate_check_query = "select ID from $wpdb->posts where ($whereCondition) and (post_type = '{$data_array['post_type']}') and (post_status != 'trash') order by ID DESC";
						$is_update = true;	
...
#Replacing post title instead of ID for post_parent added by sajitha
						if(!empty($data_array['post_parent'])){// added by priya
...
#TODO: Need to import the media for scheduler
				...
				if(isset($data_array['featured_image'])) {
					if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $data_array['featured_image'], $matchedlist, PREG_PATTERN_ORDER ) ) {
						...
						}          if(!empty($mediaConfig['img_seo_mapping_for_title']))
						{     $key=$mediaConfig['img_seo_mapping_for_title'];
                                                         $value =$tmp[$key];   
							$wpdb->query("update ".$wpdb->prefix."posts set post_title ='".$value."' where ID='".$attachid."'");
						}
						if(!empty($mediaConfig['img_seo_mapping_for_caption']))
						{   $key=$mediaConfig['img_seo_mapping_for_caption'];
                                                     $value=$tmp[$key];
							$wpdb->query("update ".$wpdb->prefix."posts set post_excerpt='".$value."' where ID='".$attachid."'");
						}
						if(!empty($mediaConfig['img_seo_mapping_for_alttext'])) {
							$key=$mediaConfig['img_seo_mapping_for_alttext'];
                                                        $value=$tmp[$key];
							$wpdb->query("insert into ".$wpdb->prefix."postmeta  (meta_key,meta_value,post_id) values('_wp_attachment_image_alt','".$value."','".$attachid."')");

						}
						if(!empty ($mediaConfig['img_seo_mapping_for_description'])) {
							$key=$mediaConfig['img_seo_mapping_for_description'];
                                                        $value=$tmp[$key];
...
public function vp_pfui_format_video_save_post($pID) {
...
public function importDataForWPMetaFields ($data_array, $pID, $importType , $core_serialize_info) {
	...
	if(!empty($data_array)) {
		foreach ($data_array as $custom_key => $custom_value) {
			...
			if( $importType != 'Users'){
				if(!empty($data_array['_format_video_embed'])){

					if (in_array('video', $post_formats[0])) {
						add_action('save_post', 'vp_pfui_format_video_save_post');
					} 

					$dataimg=$data_array['_format_video_embed'];
					$query = $wpdb->get_row("select term_id from ".$wpdb->prefix."terms where name='post-format-video'");
					$termid=$query->term_id; 
					$wpdb->query("insert into ".$wpdb->prefix."term_relationships(object_id,term_taxonomy_id)values('".$pID."','".$termid."')");
					$query2 = $wpdb->get_col("select count from ".$wpdb->prefix."term_taxonomy where term_id='$termid'");
					$result=$query2[0];
					$count=$result+1;
					$wpdb->query("update ".$wpdb->prefix."term_taxonomy set count='$count' where term_id='$termid'");
					$attachid = $this->set_featureimage( $dataimg, $pID );
					$a=array();
					update_post_meta($pID,'_format_audio_embed','');
					update_post_meta($pID,'_format_gallery_images',$a);
					update_post_meta($pID,'_thumbnail_id',$attachid);
					update_post_meta($pID,'_video_thumb_id',1);
					update_post_meta($pID,'_edit_last',1);
...
public function set_featureimage($f_img, $post_id, $media_handle = null){
	...
	elseif(!empty($media_handle['imageprocess']) && $media_handle['imageprocess'] == 'imagenamechangeoption'){
		...
		global $wpdb, $uci_admin;
		if(!empty($f_img)){
			$pathinfo = pathinfo($f_img);
			$rename_file =$pathinfo['filename'];
			$attach_query2='';
			$fimg_type = wp_check_filetype( $f_img, null );
			...
			$attach_query2 = $wpdb->get_results($wpdb->prepare("select ID from $wpdb->posts where post_title = %s and post_mime_type = %s",$rename_file,$mime_type));
		}
		if(!empty($attach_query2)){
			$attach_id = $attach_query2[0]->ID;
		}
 ...
 *
 * @param $get_me?dia_settings   - Media Settings
 * @param $data_array           - Data Array
 *
 * @return array
 */
public function parse_media_settings($get_media_settings, $data_array) {
	...
	// using title name to get slug
	//	$get_name = $wpdb->get_col($wpdb->prepare("select name from $gallery_table where title='$gallery_name'"));
	if(!empty($get_name)){
		$gallery_name=$get_name[0];
	}else{
		$gallery_name=$data_array['nextgen-gallery'];
	}
	//priya	
	...
	{/* for Alttext & Describtion */
		$event_info = $uci_admin->getEventInformation();
		$alttext_name=$event_info['media_handling']['img_seo_mapping_for_alttext'];
		$desc_name=$event_info['media_handling']['img_seo_mapping_for_description'];
		foreach ($event_info['csv_row_data'] as $key => $value) {
			if($key==$alttext_name){ 
				$Alttext=$value;
			}
			if($key==$desc_name){ 
		...
		// using title name to get slug
		$get_gallery_name = $wpdb->get_col($wpdb->prepare("select name from $gallery_table where title='$id'"));
		if(!empty($get_gallery_name)){
			$id=$get_gallery_name[0];
		}else{
			$id=$data_array['nextgen-gallery'];
		...
		if ($copy_image) {
			...
			if (!$attachment_id) {
				...
				$attachment = array('post_title' => $title, 'post_content' => $caption, 'post_status' => 'attachment', 'post_parent' => 0, 'post_mime_type' => $image_type, 'guid' => $url); // priya
